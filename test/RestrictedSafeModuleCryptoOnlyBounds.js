...
async function fixture() {
  ...
  return { delegate, module, now };
}

describe("RestrictedSafeModuleCryptoOnly — límites de orden", function () {
  it("rechaza una orden por encima de 50 USDT", async function () {
    const { delegate, module, now } = await fixture();
    await expect(module.connect(delegate).buildExactInputSingle(BTCB, POOL, 500, 50n * 10n ** 18n + 1n, 1n, now + 60))
      .to.be.revertedWithCustomError(module, "AmountExceedsPerOperationLimit");
  });

  it("rechaza un deadline vencido", async function () {
    const { delegate, module, now } = await fixture();
    await expect(module.connect(delegate).buildExactInputSingle(BTCB, POOL, 500, 1n, 1n, now))
      .to.be.revertedWithCustomError(module, "ExpiredDeadline");
  });

  it("rechaza un deadline superior a cinco minutos", async function () {
    const { delegate, module, now } = await fixture();
    await expect(module.connect(delegate).buildExactInputSingle(BTCB, POOL, 500, 1n, 1n, now + 1000))
      .to.be.revertedWithCustomError(module, "DeadlineTooFar");
  });
});
